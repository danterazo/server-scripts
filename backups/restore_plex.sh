...
if [[ -f "${backup_dir}/snapshot_${backup_datetime}/${plex_backup_file}" ]]; then
	...
	if [[ $REPLY =~ ^[Yy]$ ]]; then
		est_minutes=4	# TODO: sudo time sh restore_plex.sh
		...
		if [[ $REPLY =~ ^[Yy]$ ]]; then
			...
			plex_tarball_path="${tmp_dir}/plex_${backup_datetime}_restore.tar"

			## uncompress Plex 7z archive -> tarball
			7z x -sdel $plex_archive_path $plex_tarball_path
	...
	sudo find ${backup_dir} -name 'plex*.tar'
